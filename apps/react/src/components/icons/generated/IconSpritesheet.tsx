// WARNING: generated file! See 'yarn gen:icons'. Do not modify!
export const IconSpritesheet = (props: any) => (
	<svg
		xmlns="http://www.w3.org/2000/svg"
		style={{ display: 'none' }}
		{...props}
	>
		<defs>
			<symbol id="icon-placeholder" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_9_41)">
					<path
						d="M3.75005 3.75L11.25 11.25M11.25 3.75L3.75005 11.25M14.1667 7.49999C14.1667 11.1819 11.1819 14.1667 7.50004 14.1667C3.81814 14.1667 0.833374 11.1819 0.833374 7.49999C0.833374 3.8181 3.81814 0.833328 7.50004 0.833328C11.1819 0.833328 14.1667 3.8181 14.1667 7.49999Z"
						stroke="currentColor"
					/>
				</g>
				<defs>
					<clipPath id="clip0_9_41">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-dots" viewBox="0 0 15 15">
				<path
					d="M3.875 7.5C3.875 7.91421 3.53921 8.25 3.125 8.25C2.71079 8.25 2.375 7.91421 2.375 7.5C2.375 7.08579 2.71079 6.75 3.125 6.75C3.53921 6.75 3.875 7.08579 3.875 7.5ZM8.25 7.5C8.25 7.91421 7.91421 8.25 7.5 8.25C7.08579 8.25 6.75 7.91421 6.75 7.5C6.75 7.08579 7.08579 6.75 7.5 6.75C7.91421 6.75 8.25 7.08579 8.25 7.5ZM12.625 7.5C12.625 7.91421 12.2892 8.25 11.875 8.25C11.4608 8.25 11.125 7.91421 11.125 7.5C11.125 7.08579 11.4608 6.75 11.875 6.75C12.2892 6.75 12.625 7.08579 12.625 7.5Z"
					stroke="currentColor"
				/>
			</symbol>
			<symbol id="icon-globe" viewBox="0 0 15 15">
				<mask
					id="mask0_301_239"
					style={{ maskType: 'alpha' }}
					maskUnits="userSpaceOnUse"
					x="1"
					y="1"
					width="13"
					height="13"
				>
					<circle cx="7.5" cy="7.5" r="6.5" fill="#D9D9D9" />
				</mask>
				<g mask="url(#mask0_301_239)">
					<circle cx="7.5" cy="7.5" r="6" stroke="currentColor" />
					<path
						d="M14.5 7.5C14.5 8.15685 13.9097 8.90677 12.5919 9.52178C11.3146 10.1179 9.51485 10.5 7.5 10.5C5.48515 10.5 3.68543 10.1179 2.40814 9.52178C1.09025 8.90677 0.5 8.15685 0.5 7.5C0.5 6.84315 1.09025 6.09323 2.40814 5.47822C3.68543 4.88215 5.48515 4.5 7.5 4.5C9.51485 4.5 11.3146 4.88215 12.5919 5.47822C13.9097 6.09323 14.5 6.84315 14.5 7.5Z"
						stroke="currentColor"
					/>
					<path
						d="M10.5 7.5C10.5 9.22874 10.1216 10.769 9.53464 11.8591C8.93669 12.9696 8.19442 13.5 7.5 13.5C6.80558 13.5 6.06331 12.9696 5.46536 11.8591C4.87838 10.769 4.5 9.22874 4.5 7.5C4.5 5.77126 4.87838 4.23096 5.46536 3.14086C6.06331 2.03038 6.80558 1.5 7.5 1.5C8.19442 1.5 8.93669 2.03038 9.53464 3.14086C10.1216 4.23096 10.5 5.77126 10.5 7.5Z"
						stroke="currentColor"
					/>
					<path d="M1 7.5H14" stroke="currentColor" />
					<path d="M7.5 1V14" stroke="currentColor" />
				</g>
			</symbol>
			<symbol id="icon-cart" viewBox="0 0 15 15">
				<path
					d="M14 2H12.9837C12.5644 2 12.1896 2.26159 12.045 2.65518L11.6923 3.61538M11.6923 3.61538L9.95496 8.34482C9.81038 8.73841 9.4356 9 9.01629 9H8.46154H6.84615H5.23077H4.45005C4.01239 9 3.62558 8.7154 3.49535 8.29757L2.40952 4.81388C2.20738 4.16533 2.69685 3.5083 3.37613 3.51638L11.6923 3.61538Z"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
				/>
				<circle cx="4" cy="12" r="0.5" stroke="currentColor" />
				<circle cx="10" cy="12" r="0.5" stroke="currentColor" />
			</symbol>
			<symbol id="icon-fridge" viewBox="0 0 15 15">
				<rect
					x="3.5"
					y="0.5"
					width="8"
					height="14"
					rx="1.5"
					stroke="currentColor"
				/>
				<path d="M3.5 6H11.5" stroke="currentColor" strokeLinecap="square" />
				<path
					d="M9.5 8V9.75V11.5"
					stroke="currentColor"
					strokeLinecap="round"
				/>
			</symbol>
			<symbol id="icon-book" viewBox="0 0 15 15">
				<rect
					x="2.5"
					y="1.5"
					width="10"
					height="12"
					rx="0.5"
					stroke="currentColor"
				/>
				<path d="M10.5 1.5V13.5" stroke="currentColor" strokeLinecap="round" />
				<rect
					x="4.5"
					y="4.5"
					width="4"
					height="2"
					rx="0.5"
					stroke="currentColor"
				/>
			</symbol>
			<symbol id="icon-flag" viewBox="0 0 15 15">
				<path
					d="M8.5 14.5L10.6429 4.99999M10.6429 4.99999L11.5 0.999986C11.5 0.999986 9 1.99999 7.5 0.999986C6 -1.35601e-05 3.5 0.999986 3.5 0.999986L3 4.99999C3 4.99999 5 3.49999 6.8258 4.60521C8.6516 5.71043 10.6429 4.99999 10.6429 4.99999Z"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
				/>
			</symbol>
			<symbol id="icon-locate" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_401_232)">
					<circle cx="7.5" cy="7.5" r="5" stroke="currentColor" />
					<circle cx="7.5" cy="7.5" r="1.5" fill="currentColor" />
					<path
						d="M4 7.5H0.5M11 7.5H14.5M7.5 4V0.5M7.5 11V14.5"
						stroke="currentColor"
						strokeLinecap="round"
					/>
				</g>
				<defs>
					<clipPath id="clip0_401_232">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-tag" viewBox="0 0 15 15">
				<path
					d="M2 7V2H7L14 9L9 14L2 7Z"
					stroke="currentColor"
					strokeLinejoin="round"
				/>
				<circle cx="4.5" cy="4.5" r="1" fill="currentColor" />
			</symbol>
			<symbol id="icon-scan" viewBox="0 0 15 15">
				<path
					d="M12.5 4.5V14.5H2.5V0.5H8.5M12.5 4.5L8.5 0.5M12.5 4.5H8.5V0.5"
					stroke="currentColor"
					strokeLinejoin="round"
				/>
				<circle cx="7" cy="8" r="2.5" stroke="currentColor" />
				<path d="M9 10L12.5 13.5" stroke="currentColor" strokeLinecap="round" />
			</symbol>
			<symbol id="icon-filter" viewBox="0 0 15 15">
				<path
					d="M13 2.5H2L6.5 9V13.5L8.5 12.5V9L13 2.5Z"
					stroke="currentColor"
					strokeLinejoin="round"
				/>
			</symbol>
			<symbol id="icon-upload" viewBox="0 0 15 15">
				<path
					d="M1.5 10V11.5C1.5 12.6046 2.39543 13.5 3.5 13.5H11.5C12.6046 13.5 13.5 12.6046 13.5 11.5V10M7.5 10.5V1M7.5 1L4 4M7.5 1L11 4"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
				/>
			</symbol>
			<symbol id="icon-add_note" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_604_45)">
					<path
						d="M9.5 0.5H0.5V14.5H14.5V5.5M9.5 0.5L14.5 5.5M9.5 0.5V5.5H14.5M7.5 3.5V11.5M3.5 7.5H11.5"
						stroke="currentColor"
						strokeLinecap="round"
					/>
				</g>
				<defs>
					<clipPath id="clip0_604_45">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-drag_vertical" viewBox="0 0 15 15">
				<path
					d="M1 7.5H7.5M14 7.5H7.5M7.5 7.5V1.5M7.5 7.5V13.5M7.5 1.5L5.5 3.5M7.5 1.5L9.5 3.5M7.5 13.5L5.5 11.5M7.5 13.5L9.5 11.5"
					stroke="currentColor"
					strokeLinecap="round"
					strokeLinejoin="round"
				/>
			</symbol>
			<symbol id="icon-note" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_610_45)">
					<path
						d="M9.5 0.5H0.5V14.5H14.5V5.5M9.5 0.5L14.5 5.5M9.5 0.5V5.5H14.5"
						stroke="currentColor"
					/>
				</g>
				<defs>
					<clipPath id="clip0_610_45">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-food" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_611_45)">
					<path
						d="M7.49992 0.5V2M7.49992 4.5V2M7.49992 4.5C3.5 3 2 4 1.49994 6.5C0.999871 9 2.99993 13.5 4.49994 14C5.99994 14.5 6.99992 13.5 7.49992 13.5C7.99992 13.5 9.49992 14.5 10.4999 14C11.4999 13.5 13.9999 9 13.4999 6.5C13 4 11.6173 3.03149 7.49992 4.5ZM7.49992 2C7.49992 2 9.49992 0.499977 10.4999 0.5C11.0522 0.500013 11.6746 0.976057 11.4999 1.5C11.3618 1.91421 11.4999 2 10.4999 2C9.32835 2 7.49992 2 7.49992 2Z"
						stroke="currentColor"
						strokeLinecap="round"
					/>
				</g>
				<defs>
					<clipPath id="clip0_611_45">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-magic" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_613_45)">
					<path
						d="M7.5 11V14M6 12.5H9M5.5 1.5C4.5 3.5 3.5 4.5 1.5 5.5C3.5 6.5 4.5 7.5 5.5 9.5C6.5 7.5 7.5 6.5 9.5 5.5C7.5 4.5 6.5 3.5 5.5 1.5ZM11.5 6.5C11 7.5 10.5 8 9.5 8.5C10.5 9 11 9.5 11.5 10.5C12 9.5 12.5 9 13.5 8.5C12.5 8 12 7.5 11.5 6.5Z"
						stroke="currentColor"
						strokeLinecap="round"
					/>
				</g>
				<defs>
					<clipPath id="clip0_613_45">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-convert" viewBox="0 0 15 15">
				<path
					d="M2.5 7V9.5C2.5 12 5 11.5 7.5 11.5M7.5 11.5L5.5 9.5M7.5 11.5L5.5 13.5M12.5 8L12.5 5.5C12.5 3 10 3.5 7.5 3.5M7.5 3.5L9.5 5.5M7.5 3.5L9.5 1.5M13.5 9.5H9.5V13.5H13.5V9.5ZM5.5 3.5C5.5 2.39543 4.60457 1.5 3.5 1.5C2.39543 1.5 1.5 2.39543 1.5 3.5C1.5 4.60457 2.39543 5.5 3.5 5.5C4.60457 5.5 5.5 4.60457 5.5 3.5Z"
					stroke="currentColor"
					strokeLinecap="round"
				/>
			</symbol>
			<symbol id="icon-offline" viewBox="0 0 15 15">
				<g clipPath="url(#clip0_701_39)">
					<mask
						id="mask0_701_39"
						style={{ maskType: 'alpha' }}
						maskUnits="userSpaceOnUse"
						x="0"
						y="0"
						width="15"
						height="15"
					>
						<circle cx="7.5" cy="7.5" r="7" fill="#D9D9D9" />
					</mask>
					<g mask="url(#mask0_701_39)">
						<path
							d="M14 7.5C14 11.0899 11.0899 14 7.5 14M14 7.5C14 3.91015 11.0899 1 7.5 1M14 7.5H1M7.5 14C3.91015 14 1 11.0899 1 7.5M7.5 14C9.433 14 11 11.0899 11 7.5C11 3.91015 9.433 1 7.5 1M7.5 14C5.567 14 4 11.0899 4 7.5C4 3.91015 5.567 1 7.5 1M7.5 14V1M1 7.5C1 3.91015 3.91015 1 7.5 1M15 7.5C15 9.433 11.6421 11 7.5 11C3.35786 11 0 9.433 0 7.5C0 5.567 3.35786 4 7.5 4C11.6421 4 15 5.567 15 7.5Z"
							stroke="currentColor"
						/>
					</g>
					<circle cx="10.5" cy="10.5" r="4" fill="#FD7E4D" />
					<path d="M8 13L13 8M8 8L13 13" stroke="currentColor" />
				</g>
				<defs>
					<clipPath id="clip0_701_39">
						<rect width="15" height="15" fill="white" />
					</clipPath>
				</defs>
			</symbol>
			<symbol id="icon-profile" viewBox="0 0 15 15">
				<circle cx="7.5" cy="3.5" r="3" stroke="currentColor" />
				<path d="M1.5 15C1.5 6.99998 13.5 7 13.5 15" stroke="currentColor" />
			</symbol>
		</defs>
	</svg>
);
